...
& $PythonExe $InstalledWorker texture-health --json
if ($LASTEXITCODE -ne 0) {
    throw "Texture health command failed with exit code $LASTEXITCODE."
}

$HealthLines = @(& $PythonExe $InstalledWorker texture-health --json)
