...
Copy-Item -Force (Join-Path $RepoRoot "scripts\setup\windows-hunyuan-texture.ps1") (Join-Path $SetupDir "windows-hunyuan-texture.ps1")
...
$TextureSetupLauncher = @'
@echo off
setlocal
cd /d "%~dp0"
powershell.exe -NoProfile -ExecutionPolicy Bypass -File "%~dp0scripts\setup\windows-hunyuan-texture.ps1"
pause
'@
Set-Content -Path (Join-Path $OutputDir "setup-texture-runtime.cmd") -Value $TextureSetupLauncher -Encoding ASCII
...
Write-Host "Portable texture test package ready: $OutputDir" -ForegroundColor Green
